...
func TestInferErrorCode(t *testing.T) {
	cases := []struct {
		...
		{name: "user setting value", status: http.StatusBadRequest, msg: "invalid value for chat.file_mode: must be one of 'auto', 'rag'", want: "user_settings.invalid_value"},		{name: "third party login disabled", status: http.StatusBadRequest, msg: "third-party login is disabled", want: "auth.provider_login_disabled"},
