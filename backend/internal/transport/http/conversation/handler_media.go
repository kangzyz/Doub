...
// streamMediaVideo 只负责 HTTP 绑定和 NDJSON 事件转发，视频业务由 application 执行。
func (h *Handler) streamMediaVideo(c *gin.Context) {
	userID := middleware.MustUserID(c)
	publicID, err := stringParam(c, "id")
	if err != nil {
		response.Error(c, http.StatusBadRequest, "invalid conversation id")
	...
	result, err := h.service.StreamMediaVideo(c.Request.Context(), appconversation.MediaVideoInput{
		UserID:                userID,
		ConversationID:        conversation.ID,
		RequestID:             middleware.MustRequestID(c),
		Prompt:                req.Prompt,
		...
		ClientRunID:           req.ClientRunID,
		FileIDs:               req.FileIDs,
		InputReferenceFileID:  req.InputReferenceFileID,
		ParentMessagePublicID: req.ParentMessagePublicID,
		SourceMessagePublicID: req.SourceMessagePublicID,
		BranchReason:          req.BranchReason,
		OnEvent: func(eventType string, payload map[string]interface{}) error {
