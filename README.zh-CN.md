...
<p align="center">
  面向企业模型路由、多模态对话、文件、工具、身份和运维的一体化 AI 工作台。
</p>
...
DOUB Chat 为团队提供统一的 AI 工作台，用一个清晰的使用入口承载多个上游模型和服务商。它将多模态对话、模型路由、文件与 RAG、MCP 工具、身份认证、审计日志和运维控制整合到同一个产品中。

系统围绕简单部署、高效静态分发和可预期的 Go 运行时占用设计。后台集中管理上游渠道、平台模型名、路由优先级、用户和安全策略，对话工作区则保持稳定、专注的用户体验。
...
| 后台管理 | 用户、角色、身份源、上游、平台模型、路由绑定、审计日志、认证事件、系统事件和运行时设置。 |
...
数据库使用领域前缀组织身份、模型路由、对话、文件、RAG、设置、工具、审计日志和系统事件等表。审计日志、系统事件和高增长向量数据保持独立事实源。
...
默认应用镜像为 `ghcr.io/kangzyz/doub-chat:latest`。测试自定义构建时可通过 `DOUB_CHAT_IMAGE` 覆盖：
...
下面的 compose 文件会接入 `doub-chat-network`。可先执行 `docker network create doub-chat-network`，或先启动一次根目录 compose。
