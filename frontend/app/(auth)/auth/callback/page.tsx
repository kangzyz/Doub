    ...
    void completeProviderLogin(provider, code, state, redirectURI, codeVerifier, intent)
      .then((result) => {
        if (result.twoFactorRequired) {
          window.sessionStorage.setItem(twoFactorChallengeStorageKey, result.twoFactorChallengeToken ?? "");
          window.sessionStorage.setItem(twoFactorMethodsStorageKey, JSON.stringify(result.verificationMethods ?? ["two_factor"]));
...
function parseProviderState(raw: string): { next: string; intent: "login" | "register" | "bind" } {
  try {
    ...
    if (!encodedPayload) return { next: "/chat", intent: "login" };
